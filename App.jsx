...
const SLIDES = [
  {title:"Compra en USA.", em:"Recíbelo en tu país.", sub:"Accede a miles de tiendas.",video:"https://raw.githubusercontent.com/bicplobelramirez-ai/USAlinkRD/main/istockphoto-2174935798-640_adpp_is-2.mp4", emoji:"✈️", bg:"linear-gradient(145deg,#050f2b,#081b4b)", cta:"Ver tiendas", page:"stores"},
  ...
  return (
    <div style={{margin:"16px 16px 0"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:NAVY}}>🔥 Ofertas del Día</span>
...
function FeaturedStores({onNavigate,onStoreSelect}){
  ...
  return (
    <div style={{margin:"16px 16px 0"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {featured.map((s,i)=>(
           <div key={s.id} onClick={()=>onStoreSelect(s)} style={{background:"#fff",border:"1.5px solid #dde2f0",borderRadius:16,padding:"16px 14px",cursor:"pointer",boxShadow:"0 2px 10px rgba(8,27,75,0.06)",position:"relative"}}>
...
{Object.entries(GEO_RATES).map(([k,v])=><option key={k} value={v.rate}>{v.flag} {v.name}</option>)}
        ...
        {result&&(
          <div style={{marginTop:14,background:"linear-gradient(135deg,#081B4B,#050f2b)",borderRadius:14,padding:16}}>
           {[["Precio","$"+result.price],["Envío","$"+result.shipping],["Manejo","$"+result.handling],["Servicio USALINK (8%)","$"+result.service]].map(([l,v])=>( 
...
function LinkPage({geo}){
  ...
  setChat(c=>[...c,{bot:false,text:link},{bot:true,text:"¡Recibido! ✅ Ya tengo tu producto. Para darte el precio exacto y confirmar disponibilidad, te conecto directo con nuestro equipo por WhatsApp — te responden en minutos."}]);
  setLastLink(link);
