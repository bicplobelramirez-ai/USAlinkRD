...
const STORES = [
  ...
  {id:"rlfactory",name:"RL Factory",cat:"outlets",bg:"linear-gradient(135deg,#002a5e,#1a4a8a)",badge:"sale",rating:4.7,url:"https://www.ralphlauren.com/factory-stores",desc:"Ralph Lauren Factory a precio reducido."},
...
function Av({size=40,pulse=false}){
  return (
    <div style={{position:"relative",width:size,height:size,flexShrink:0}}>
      <div style={{width:size,height:size,borderRadius:"50%",background:"linear-gradient(135deg,#E31E24,#ff6b9d,#081B4B)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*0.45,border:"2px solid rgba(255,255,255,0.2)"}}>
        👩🏻‍💼
      </div>
  ...
  return (
    <div onClick={()=>onClick(store)} style={{background:"#fff",borderRadius:20,border:"1.5px solid #dde2f0",overflow:"hidden",cursor:"pointer",boxShadow:"0 2px 12px rgba(8,27,75,0.07)"}}>
      <div style={{height:100,background:store.bg,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
        ...
        <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:"#fff",letterSpacing:"0.04em",position:"relative",zIndex:2,textAlign:"center",padding:"0 8px"}}>{store.name}</span>
...
function HeroSlider({onNavigate}){
  ...
  return (
    <div style={{height:200,position:"relative",overflow:"hidden",background:s.bg}}
      ...
      <div style={{position:"absolute",top:18,right:18,fontSize:44,zIndex:2}}>{s.emoji}</div>
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"0 20px 18px",zIndex:3}}>
        ...
        <button onClick={()=>onNavigate(s.page)} style={{background:RED,color:"#fff",fontSize:11,fontWeight:800,padding:"9px 16px",borderRadius:999,border:"none",cursor:"pointer"}}>
          {s.cta} →
        </button>
      </div>
...
function StoreDetail({store,geo,onBack,onNavigate}){
  ...
  const [chat,setChat]=useState([{bot:true,text:"¡Hola! Soy Aphrodite 👩🏻‍💼 ¿Qué te interesa comprar en "+store.name+"?"}]);
  ...
  const send=()=>{
    ...
    callAI(msgs,"Eres Aphrodite de USALINK. Tienda: "+store.name+". Tarifa: $"+rate+"/lb. Español amigable, máx 3 oraciones.",
      r=>{setChat(c=>[...c,{bot:true,text:r}]);setLoading(false);setTimeout(()=>{if(ref.current)ref.current.scrollTop=ref.current.scrollHeight;},100);},
    ...
    <div>
      <div style={{height:180,background:store.bg,position:"relative",display:"flex",alignItems:"flex-end"}}>
        ...
        <div style={{position:"relative",zIndex:2,padding:"0 18px 18px"}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:40,color:"#fff"}}>{store.name}</div>
      ...
      <div style={{display:"flex",background:"#F3F5FB",borderBottom:"1px solid #dde2f0"}}>
        {[["✈","$"+rate+"/lb","Envío"],["⏱","3-7 días","Entrega"],["🛡️","Incluido","Seguro"],["⭐",store.rating,"Rating"]].map(([icon,val,lbl])=>(
          <div key={lbl} style={{flex:1,textAlign:"center",padding:"12px 4px",borderRight:"1px solid #dde2f0"}}>
            ...
            <div style={{fontSize:9,color:"#8b96b8",textTransform:"uppercase"}}>{lbl}</div>
          </div>
        ))}
      </div>
      <div style={{padding:"14px 16px",background:"#fff",borderBottom:"1px solid #dde2f0"}}>
        <div style={{fontSize:13,fontWeight:800,color:NAVY,marginBottom:6}}>Sobre {store.name}</div>
        <div style={{fontSize:13,color:"#5d6a8e",lineHeight:1.65}}>{store.desc}</div>
      </div>
      <div style={{margin:"14px 16px",background:"linear-gradient(135deg,#081B4B,#0a2070)",borderRadius:20,padding:16}}>
        ...
        <div ref={ref} style={{maxHeight:140,overflowY:"auto",marginBottom:10,display:"flex",flexDirection:"column",gap:8}}>
          {chat.map((m,i)=>(
          ...
          {loading&&<div style={{display:"flex",gap:6}}><Av size={22}/><div style={{background:"rgba(255,255,255,0.1)",borderRadius:14,padding:"9px 12px",display:"flex",gap:3}}>{[0,1,2].map(i=><span key={i} style={{width:5,height:5,borderRadius:"50%",background:"rgba(255,255,255,0.5)",display:"inline-block"}}/>)}</div></div>}
        </div>
        <div style={{display:"flex",gap:8}}>
          <input value={msg} onChange={e=>setMsg(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Pregunta algo..." style={{flex:1,background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:999,padding:"9px 14px",fontSize:13,color:"#fff",outline:"none"}}/>
          <button onClick={send} style={{width:38,height:38,borderRadius:"50%",background:RED,color:"#fff",border:"none",cursor:"pointer",fontSize:14,flexShrink:0}}>➤</button>
        </div>
      </div>
      <div style={{height:16}}/>
    </div>
...
function HomePage({onNavigate,geo}){
  return (
    <div>
      ...
      {geo&&<div style={{background:"linear-gradient(90deg,#081B4B,#0d2560)",padding:"8px 16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{fontSize:12,color:"rgba(255,255,255,0.8)",fontWeight:600}}>{geo.flag} {geo.name} · ✈ ${geo.rate}/lb</div>
        <button onClick={()=>onNavigate("link")} style={{background:"rgba(227,30,36,0.2)",border:"1px solid rgba(227,30,36,0.4)",borderRadius:999,padding:"5px 12px",fontSize:11,fontWeight:700,color:RED,cursor:"pointer"}}>🔗 Pegar link</button>
      </div>}
      ...
      <div style={{padding:"16px 16px 0"}}>
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:8}}>
          {[["🛍️","Outlets","outlets"],["👗","Moda","moda"],["🏬","Markets","marketplaces"],["👟","Calzado","calzado"],["💄","Belleza","belleza"],["📱","Tech","tech"],["🏠","Hogar","hogar"]].map(([icon,lbl,cat])=>(
            <button key={cat} onClick={()=>onNavigate("stores",cat)} style={{flexShrink:0,background:"#fff",border:"1.5px solid #dde2f0",borderRadius:20,padding:"8px 14px",display:"flex",alignItems:"center",gap:7,fontSize:12,fontWeight:700,color:NAVY,cursor:"pointer",whiteSpace:"nowrap"}}>
      ...
      <div style={{display:"flex",background:"linear-gradient(135deg,#081B4B,#0a1f55)",margin:"14px 0 0"}}>
        {[["+50K","Clientes"],["+1M","Paquetes"],["+20","Países"],["99%","Satisfacción"]].map(([num,lbl])=>(
          <div key={lbl} style={{flex:1,textAlign:"center",padding:"14px 6px",borderRight:"1px solid rgba(255,255,255,0.08)"}}>
            ...
            <div style={{fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.45)",textTransform:"uppercase"}}>{lbl}</div>
          </div>
        ))}
      </div>
    ...
    <div>
      <div style={{background:"#0d2560",padding:"10px 14px 12px"}}>
        <div style={{position:"relative"}}>
          <span style={{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",color:"rgba(255,255,255,0.35)"}}>🔍</span>
      ...
      <div style={{display:"flex",gap:7,padding:"10px 14px",overflowX:"auto",background:"#fff",borderBottom:"1px solid #dde2f0"}}>
        {CATS.map(c=>(
          <button key={c.key} onClick={()=>setCat(c.key)} style={{flexShrink:0,padding:"7px 14px",borderRadius:999,fontSize:12,fontWeight:600,border:"none",cursor:"pointer",background:cat===c.key?NAVY:"#eef0f8",color:cat===c.key?"#fff":"#5d6a8e",whiteSpace:"nowrap"}}>
            {c.icon} {c.label}
          </button>
        ))}
      </div>
...
function AIPage({geo}){
  ...
  const SYS={
    shop:"Eres Aphrodite de USALINK, asistente de compras. País: "+country+". Tarifa: "+rate+". Español amigable, máx 3 párrafos.",
    support:"Eres Aphrodite de USALINK, soporte. Envíos 3-7 días. País: "+country+". Tarifa: "+rate+". Español, máx 3 párrafos.",
    quote:"Eres Aphrodite, cotizadora. Calcula: peso × tarifa + $3 manejo. Tarifas: RD $8.50, MX $9.00, CO $9.50, VE $10.00, PE $10.50, CL $11.00, AR $11.50, BR $12.00.",
    link:"Eres Aphrodite de USALINK. Analiza links USA y calcula costo total con envío a "+country+" a "+rate+". Español.",
    agent:"Eres Aphrodite, agente proactivo USALINK. Cotiza, recomienda, analiza links. País: "+country+". Español, máx 4 párrafos.",
  };
  const WELCOME={
    shop:"¡Hola! 👋 Soy Aphrodite, tu guía de compras en USA. ¿Qué buscás hoy? 🛍️",
    support:"¡Hola! Estoy aquí para resolver tus dudas 💙",
    quote:"¡Hola! Dame el peso y te doy el precio exacto para "+country+" 📦",
    link:"¡Hola! 🔗 Pegame el link de cualquier producto USA.",
    agent:"¡Hola! Soy tu agente proactiva ⚡ ¿En qué te ayudo?",
  };
  const send=(msg)=>{
    if(!msg.trim()||loading)return;
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      ...
      <div ref={ref} style={{flex:1,overflowY:"auto",padding:"14px 14px 8px",display:"flex",flexDirection:"column",gap:12,background:"#F3F5FB"}}>
        {chat.length===0&&(
          <div style={{background:"#fff",borderRadius:20,padding:18,display:"flex",gap:12,alignItems:"flex-start"}}>
            <Av size={40} pulse/>
        ...
        {chat.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:m.bot?"flex-start":"flex-end",gap:8,alignItems:"flex-end"}}>
            {m.bot&&<Av size={30}/>}
            <div style={{maxWidth:"78%",padding:"11px 14px",borderRadius:18,background:m.bot?"#fff":NAVY,color:m.bot?"#1a2240":"#fff",fontSize:13,lineHeight:1.6}}>{m.text}</div>
          </div>
        ))}
        {loading&&(
          <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
            <Av size={30}/>
            <div style={{background:"#fff",borderRadius:18,padding:"12px 16px"}}>
              <div style={{display:"flex",gap:4,alignItems:"center"}}>
                {[0,1,2].map(i=><span key={i} style={{width:7,height:7,borderRadius:"50%",background:RED,display:"inline-block"}}/>)}
      ...
      <div style={{display:"flex",gap:8,padding:"10px 14px",background:"#fff",borderTop:"1px solid #dde2f0",flexShrink:0}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send(input)} placeholder="Pregúntame algo..." style={{flex:1,padding:"10px 14px",border:"1.5px solid #dde2f0",borderRadius:999,fontSize:14,outline:"none"}}/>
        <button onClick={()=>send(input)} style={{width:40,height:40,borderRadius:"50%",background:RED,color:"#fff",border:"none",cursor:"pointer",fontSize:14,flexShrink:0}}>➤</button>
      </div>
    </div>
  );
}
...
function TrackPage(){
  const [id,setId]=useState(""),  [res,setRes]=useState(null);
  return (
    <div>
      <div style={{background:"linear-gradient(155deg,#050f2b,#0a1f55)",padding:"28px 20px 32px"}}>
        ...
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:34,color:"#fff",marginBottom:16}}>Rastrea tu paquete</div>
        <div style={{display:"flex",gap:8}}>
          <input value={id} onChange={e=>setId(e.target.value)} onKeyDown={e=>e.key==="Enter"&&id.trim().length>=3&&setRes(id.toUpperCase())} placeholder="USL-2024-XXXXXXX" style={{flex:1,background:"rgba(255,255,255,0.09)",border:"1.5px solid rgba(255,255,255,0.15)",borderRadius:999,padding:"13px 16px",fontSize:13,color:"#fff",outline:"none",fontFamily:"monospace"}}/>
          <button onClick={()=>id.trim().length>=3&&setRes(id.toUpperCase())} style={{background:RED,color:"#fff",fontSize:13,fontWeight:800,padding:"13px 20px",borderRadius:999,border:"none",cursor:"pointer",flexShrink:0}}>Buscar</button>
        </div>
        {res&&(
          <div style={{marginTop:18,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:18}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingBottom:14,borderBottom:"1px solid rgba(255,255,255,0.1)",marginBottom:14}}>
              <span style={{fontFamily:"monospace",fontSize:12,color:"rgba(255,255,255,0.55)"}}>{res}</span>
            ...
            {[["done","✅ Recibido en Miami","28 May 2026"],["done","✅ Procesado","29 May 2026"],["now","✈ En vuelo","En progreso"],["","📋 Aduana","1 Jun"],["","🏠 Entrega","2 Jun"]].map(([st,lbl,date],i)=>(
              <div key={i} style={{display:"flex",gap:12,marginBottom:12}}>
                ...
                <div><div style={{fontSize:12,fontWeight:700,color:"#fff"}}>{lbl}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.38)"}}>{date}</div></div>
              </div>
            ))}
          </div>
        )}
...
function CalcPage({geo}){
  const [w,setW]=useState(""), [c,setC]=useState(geo?String(geo.rate):""), [res,setRes]=useState(null);
  const calc=()=>{
    const wn=parseFloat(w)||0, rn=parseFloat(c)||0;
    if(!wn||!rn){alert("Completa todos los campos");return;}
    setRes({b:(wn*rn).toFixed(2),t:(wn*rn+3).toFixed(2)});
  };
  return (
    <div>
      ...
      <div style={{margin:"14px 16px",background:"#fff",borderRadius:20,padding:20,border:"1.5px solid #dde2f0"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
          <div>
            <label style={{fontSize:11,fontWeight:700,color:"#5d6a8e",textTransform:"uppercase",display:"block",marginBottom:7}}>Peso (lbs)</label>
            <input type="number" value={w} onChange={e=>setW(e.target.value)} placeholder="0.0" style={{width:"100%",padding:"11px 14px",border:"1.5px solid #dde2f0",borderRadius:14,fontSize:14,color:NAVY,outline:"none",boxSizing:"border-box"}}/>
          </div>
          <div>
            <label style={{fontSize:11,fontWeight:700,color:"#5d6a8e",textTransform:"uppercase",display:"block",marginBottom:7}}>País</label>
            <select value={c} onChange={e=>setC(e.target.value)} style={{width:"100%",padding:"11px 14px",border:"1.5px solid #dde2f0",borderRadius:14,fontSize:13,color:NAVY,outline:"none",background:"#fff",WebkitAppearance:"none",boxSizing:"border-box"}}>
              <option value="">Seleccionar...</option>
              {Object.entries(GEO_RATES).map(([k,v])=><option key={k} value={v.rate}>{v.flag} {v.name}</option>)}
            </select>
          </div>
        </div>
        <button onClick={calc} style={{width:"100%",padding:15,borderRadius:14,background:RED,color:"#fff",fontSize:14,fontWeight:800,border:"none",cursor:"pointer"}}>📦 Calcular</button>
        {res&&(
          <div style={{marginTop:14,background:"linear-gradient(135deg,#081B4B,#050f2b)",borderRadius:14,padding:16}}>
            {[["Base","$"+res.b],["Manejo","$3.00"]].map(([l,v])=>(
            ...
            <div style={{display:"flex",justifyContent:"space-between",fontSize:16,fontWeight:800,color:"#fff",paddingTop:10,borderTop:"1px solid rgba(255,255,255,0.1)"}}>
              <span>💰 Total</span><span style={{color:"#34d399",fontSize:20}}>${res.t}</span>
            </div>
          </div>
        )}
      </div>
...
function LinkPage({geo}){
  ...
  const [chat,setChat]=useState([{bot:true,text:"¡Hola! 👋 Pegame el link de cualquier producto USA y te calculo el costo total con envío a "+(geo?geo.name:"tu país")+" 🛍️"}]);
  const [loading,setLoading]=useState(false);
  ...
  const analyze=()=>{
    ...
    callAI([{role:"user",content:"Analiza este link: "+link+"\nIdentifica la tienda y el producto. Pide precio si no está en la URL. Pide el peso para calcular envío a "+(geo?geo.name:"mi país")+" a $"+rate+"/lb."}],
      "Eres Aphrodite de USALINK. Analiza links USA y calcula costos. Español amigable.",
      r=>{setChat(c=>[...c,{bot:true,text:r}]);setLoading(false);setTimeout(()=>{if(ref.current)ref.current.scrollTop=ref.current.scrollHeight;},100);},
      ()=>{setChat(c=>[...c,{bot:true,text:"Sin conexión 🙏"}]);setLoading(false);}
    );
  };
  const sendFu=()=>{
    if(!fu.trim()||loading)return;
    const m=fu.trim();setFu("");
    const nc=[...chat,{bot:false,text:m}];setChat(nc);setLoading(true);
    const msgs=nc.map(c=>({role:c.bot?"assistant":"user",content:c.text}));
    callAI(msgs,"Eres Aphrodite de USALINK. Ayudas a calcular costo de producto USA con envío a "+(geo?geo.name:"Latinoamérica")+" a $"+rate+"/lb. Español.",
      r=>{setChat(c=>[...c,{bot:true,text:r}]);setLoading(false);setTimeout(()=>{if(ref.current)ref.current.scrollTop=ref.current.scrollHeight;},100);},
      ()=>{setChat(c=>[...c,{bot:true,text:"Sin conexión 🙏"}]);setLoading(false);}
    );
  };
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      <div style={{background:"linear-gradient(135deg,#081B4B,#1a0a3d)",padding:"20px 18px 18px",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          <Av size={46} pulse/>
          <div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:"#fff"}}>Pega el Link</div>
        ...
        <div style={{background:"rgba(255,255,255,0.08)",border:"1.5px solid rgba(255,255,255,0.15)",borderRadius:16,padding:"12px 14px"}}>
          ...
          <button onClick={analyze} disabled={!url.trim()||loading} style={{width:"100%",padding:"11px",borderRadius:12,background:url.trim()?RED:"rgba(255,255,255,0.1)",color:"#fff",fontSize:13,fontWeight:800,border:"none",cursor:url.trim()?"pointer":"default"}}>
      ...
      <div ref={ref} style={{flex:1,overflowY:"auto",padding:"14px 14px 8px",display:"flex",flexDirection:"column",gap:12,background:"#F3F5FB"}}>
        {chat.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:m.bot?"flex-start":"flex-end",gap:8,alignItems:"flex-end"}}>
            {m.bot&&<Av size={28}/>}
            <div style={{maxWidth:"80%",padding:"11px 14px",borderRadius:18,background:m.bot?"#fff":NAVY,color:m.bot?"#1a2240":"#fff",fontSize:13,lineHeight:1.6,wordBreak:"break-all"}}>{m.text}</div>
          </div>
        ))}
        {loading&&<div style={{display:"flex",gap:8,alignItems:"flex-end"}}><Av size={28}/><div style={{background:"#fff",borderRadius:18,padding:"12px 16px"}}><div style={{display:"flex",gap:4}}>{[0,1,2].map(i=><span key={i} style={{width:7,height:7,borderRadius:"50%",background:RED,display:"inline-block"}}/>)}</div></div></div>}
      </div>
      <div style={{display:"flex",gap:8,padding:"10px 14px",background:"#fff",borderTop:"1px solid #dde2f0",flexShrink:0}}>
        <input value={fu} onChange={e=>setFu(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&fu.trim())sendFu();}} placeholder="Responde a Aphrodite..." style={{flex:1,padding:"10px 14px",border:"1.5px solid #dde2f0",borderRadius:999,fontSize:13,outline:"none"}}/>
        <button onClick={sendFu} style={{width:40,height:40,borderRadius:"50%",background:RED,color:"#fff",border:"none",cursor:"pointer",fontSize:14,flexShrink:0}}>➤</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{background:"linear-gradient(155deg,#050f2b,#0d2560)",padding:"28px 20px 32px",textAlign:"center"}}>
        <div style={{width:80,height:80,borderRadius:"50%",background:"linear-gradient(135deg,#E31E24,#0d2560)",margin:"0 auto 14px",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Bebas Neue',sans-serif",fontSize:32,color:"#fff",border:"3px solid rgba(255,255,255,0.15)"}}>U</div>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:"#fff"}}>Mi Cuenta</div>
        <div style={{fontSize:13,color:"rgba(255,255,255,0.5)",marginBottom:8}}>usuario@email.com</div>
      ...
      <div style={{display:"flex",background:"#fff",borderBottom:"1px solid #dde2f0"}}>
        {[["0","Paquetes"],["0","En camino"],["$0","Ahorrado"]].map(([num,lbl])=>(
          <div key={lbl} style={{flex:1,textAlign:"center",padding:"16px 8px",borderRight:"1px solid #dde2f0"}}>
            ...
            <div style={{fontSize:10,fontWeight:600,color:"#8b96b8",textTransform:"uppercase",marginTop:2}}>{lbl}</div>
          </div>
        ))}
      </div>
      <div style={{padding:"16px 16px 0",display:"flex",flexDirection:"column",gap:8}}>
        {[["📦","Mis paquetes","Próximamente disponible","track"],["🔗","Pegar link","Compra asistida por IA","link"],["🛍️","Tiendas USA","38 tiendas disponibles","stores"],["🧮","Calcular envío","Cotización instantánea","calc"],["🤖","Hablar con Aphrodite","Tu asistente personal","ai"]].map(([icon,title,sub,pg])=>(
          <div key={title} onClick={()=>onNavigate(pg)} style={{background:"#fff",borderRadius:16,padding:16,display:"flex",alignItems:"center",gap:14,border:"1.5px solid #dde2f0",cursor:"pointer"}}>
...
export default function App(){
  ...
  const nav=(p,cat)=>{setPage(p);if(cat)setStoreCat(cat);setStore(null);};
  const cur=store?"stores":page;
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:opsz,wght@9..40,400;9..40,600;9..40,700;9..40,800&display=swap');
      ...
      <div style={{position:"fixed",inset:0,display:"flex",flexDirection:"column",background:"#F3F5FB",fontFamily:"'DM Sans',sans-serif",overflow:"hidden"}}>
        ...
        <nav style={{flexShrink:0,height:56,background:"rgba(8,27,75,0.97)",display:"flex",alignItems:"center",padding:"0 14px",justifyContent:"space-between",zIndex:200}}>
          <div style={{display:"flex",alignItems:"center",gap:9,cursor:"pointer"}} onClick={()=>nav("home")}>
            <div style={{width:34,height:34,background:RED,borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <button onClick={()=>nav("account")} style={{background:RED,color:"#fff",fontSize:12,fontWeight:700,padding:"7px 14px",borderRadius:999,border:"none",cursor:"pointer"}}>Cuenta</button>
          </div>
        </nav>
        <div style={{flex:1,overflow:"hidden",position:"relative"}}>
          {[
            {id:"home",show:!store&&page==="home",el:<HomePage onNavigate={nav} geo={geo}/>},
            {id:"stores",show:!store&&page==="stores",el:<StoresPage initialCat={storeCat} onStoreSelect={s=>setStore(s)}/>},
            {id:"store-d",show:!!store,el:store?<StoreDetail store={store} geo={geo} onBack={()=>setStore(null)} onNavigate={nav}/>:null},
            {id:"ai",show:!store&&page==="ai",el:<AIPage geo={geo}/>},
            {id:"link",show:!store&&page==="link",el:<LinkPage geo={geo}/>},
            {id:"track",show:!store&&page==="track",el:<TrackPage/>},
            {id:"calc",show:!store&&page==="calc",el:<CalcPage geo={geo}/>},
            {id:"account",show:!store&&page==="account",el:<AccountPage onNavigate={nav} geo={geo}/>},
          ].map(({id,show,el})=>(
        ...
        <nav style={{flexShrink:0,height:62,background:"rgba(8,27,75,0.97)",borderTop:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"stretch",zIndex:200}}>
          {NAV.map(n=>{
            const active=n.id===cur;
            return (
              <button key={n.id} onClick={()=>nav(n.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,paddingTop:6,background:"none",border:"none",cursor:"pointer",color:active?"#fff":"rgba(255,255,255,0.38)",fontSize:10,fontWeight:600}}>
                {n.special
                  ?<div style={{width:44,height:44,background:"linear-gradient(135deg,"+RED+",#ff6b9d)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,marginBottom:-4}}>{n.icon}</div>
